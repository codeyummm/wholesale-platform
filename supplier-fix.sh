#!/bin/bash
echo "🔧 Supplier Enhancement Fix - Part A: Backend"

cat > backend/controllers/invoiceController.js << 'INVOICECTRL'
...
echo "✅ Invoice controller fixed (image storage + logging)"
#!/bin/bash
echo "🔧 Supplier Enhancement Fix - Part B: InvoiceScanner"
...
echo "✅ InvoiceScanner fixed (imageUrl now passed in save)"
#!/bin/bash
echo "🔧 Supplier Enhancement Fix - Part C: SupplierList"

cat > frontend/src/components/Suppliers/SupplierList.jsx << 'EOF'
import React, { useEffect, useState } from 'react';
import api from '../../utils/api';
import InvoiceScanner from '../InvoiceScanner';
import { Plus, Search, Edit, Trash2, Phone, Mail, MapPin, X, Save, FileText, Package, DollarSign, Eye, Building, Star, Camera, ArrowLeft, Download, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function SupplierList() {
  ...
  const downloadImage = (imageUrl, fileName) => {
    ...
    link.href = imageUrl;
    link.download = fileName || 'invoice.jpg';
  ...
  if (selectedSupplier) {
    ...
    return (<div>
              ...
              return (
                <div key={inv._id} style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '14px', cursor: 'pointer' }}
                    ...
                    {/* Thumbnail */}
                    <div style={{ width: '56px', height: '56px', borderRadius: '8px', background: inv.imageUrl ? 'transparent' : '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }} onClick={(e) => e.stopPropagation()}>
                      ...
                      {inv.imageUrl && <button onClick={() => downloadImage(inv.imageUrl, `invoice-${inv.invoiceNumber || inv._id}.jpg`)} style={{ padding: '6px', background: '#ecfdf5', border: 'none', borderRadius: '6px', cursor: 'pointer' }} title="Download"><Download size={16} color="#10b981" /></button>}
                    </div>
                    ...
                    <div style={{ borderTop: '1px solid #e2e8f0' }}>
                          ...
                          <div key={idx} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '0', padding: '10px 16px', borderBottom: '1px solid #f1f5f9', fontSize: '13px' }}>
                            <div>
                              ...
                              <div style={{ fontSize: '11px', color: '#94a3b8' }}>
                                {[p.storage, p.color, p.grade, p.lockStatus].filter(Boolean).join(' · ')}
                              </div>
                            </div>
                      ...
                      {inv.imageUrl && (
                        <div style={{ padding: '12px 16px', borderTop: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <img src={inv.imageUrl} alt="Invoice scan" style={{ width: '80px', height: '80px', objectFit: 'cover', borderRadius: '8px', border: '1px solid #e2e8f0', cursor: 'pointer' }} onClick={() => setViewingImage(inv.imageUrl)} />
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                              ...
                              <button onClick={() => downloadImage(inv.imageUrl, `invoice-${inv.invoiceNumber || inv._id}.jpg`)} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 10px', background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: '5px', cursor: 'pointer', fontSize: '11px', color: '#059669' }}><Download size={12} /> Download</button>
                            </div>
                          </div>
                        </div>
                      )}
      ...
      {/* Image Viewer */}
      {viewingImage && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '2rem' }} onClick={() => setViewingImage(null)}>
          <div style={{ position: 'relative', maxWidth: '90vw', maxHeight: '90vh' }} onClick={(e) => e.stopPropagation()}>
            <div style={{ position: 'absolute', top: '-16px', right: '-16px', display: 'flex', gap: '8px', zIndex: 101 }}>
              <button onClick={() => downloadImage(viewingImage, 'invoice.jpg')} style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#10b981', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}><Download size={16} color="white" /></button>
              <button onClick={() => setViewingImage(null)} style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}><X size={18} /></button>
            </div>
...
echo "✅ SupplierList done with products, thumbnails, download"

echo ""
echo "============================================"
echo "✅ SUPPLIER FIX COMPLETE!"
echo "============================================"
echo ""
echo "Fixed:"
echo "  ✅ Invoice images now saved (base64 in imageUrl field)"
echo "  ✅ Thumbnails show on invoice list"
echo "  ✅ Click invoice → expands to show all products"
echo "  ✅ Products show: brand, model, storage, color, qty, price"
echo "  ✅ Download button on each invoice"
echo "  ✅ Full-size image viewer with download button"
echo "  ✅ Products synced with inventory on save"
echo ""
echo "Restart backend: killall -9 node && cd backend && npm start"
