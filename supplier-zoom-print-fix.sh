#!/bin/bash
echo "🔧 Fixing DocViewer zoom + adding print..."

cat > frontend/src/components/Suppliers/SupplierList.jsx << 'SUPPLIEREOF'
import React, { useEffect, useState, useRef } from 'react';
import api from '../../utils/api';
import InvoiceScanner from '../InvoiceScanner';
import { Plus, Search, Edit, Trash2, Phone, Mail, MapPin, X, Save, FileText, Package, DollarSign, Eye, Building, Star, Camera, ArrowLeft, Download, ChevronDown, ChevronUp, ZoomIn, ZoomOut, ChevronRight, Printer } from 'lucide-react';
...
function b64ToBlob(dataUrl) {
  ...
  return { blob: new Blob(byteArrays, { type: mime }), mime };
}
...
function DocViewer({ dataUrl, onClose, invoiceNumber }) {
  ...
  const doDownload = () => {
    try {
      const { blob, mime } = b64ToBlob(dataUrl);
      const url = URL.createObjectURL(blob);
      const ext = mime.includes('pdf') ? 'pdf' : mime.includes('png') ? 'png' : 'jpg';
      const a = document.createElement('a');
      a.href = url; a.download = `invoice-${invoiceNumber || 'scan'}.${ext}`;
  ...
  const doPrint = () => {
    try {
      if (isPdf && blobUrl) {
        ...
        if (win) {
          win.document.write(`<html><head><title>Invoice ${invoiceNumber || ''}</title><style>@media print{body{margin:0}img{max-width:100%;height:auto}}</style></head><body><img src="${dataUrl}" onload="window.print();window.close()"/></body></html>`);
          win.document.close();
        }
      }
  ...
  return (
    <div onClick={onClose}
      style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 9999 }}>

      {/* Toolbar */}
      <div onClick={(e) => e.stopPropagation()}
        style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 16px', background: 'rgba(20,20,20,0.95)', borderRadius: '0 0 14px 14px', backdropFilter: 'blur(10px)', flexWrap: 'wrap', justifyContent: 'center' }}>

        {/* Zoom */}
        <button onClick={zoomOut} title="Zoom Out (−)"
          style={{ padding: '7px', background: '#333', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
          <ZoomOut size={18} />
        </button>
        <button onClick={resetZoom} title="Reset Zoom"
          style={{ padding: '5px 12px', background: zoom === 100 ? '#333' : '#4338ca', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: '600', minWidth: '55px', textAlign: 'center' }}>
          {zoom}%
        </button>
        ...
        <div style={{ width: '1px', height: '28px', background: '#444', margin: '0 6px' }} />

        {/* Print */}
        <button onClick={doPrint} title="Print"
          style={{ padding: '7px 14px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', fontWeight: '600' }}>
          <Printer size={16} /> Print
        </button>

        {/* Download */}
        <button onClick={doDownload} title="Download"
          style={{ padding: '7px 14px', background: '#10b981', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', fontWeight: '600' }}>
        ...
        <button onClick={onClose} title="Close (Esc)"
          style={{ padding: '7px 14px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', fontWeight: '600' }}>
          <X size={16} /> Close
        </button>
      </div>
      ...
      <div ref={containerRef} onClick={(e) => e.stopPropagation()}
        style={{ flex: 1, overflow: 'auto', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '20px', width: '100%' }}>
        ...
        {isPdf ? (
          blobUrl && <iframe
            ...
            style={{
              ...
              height: `${Math.round((window.innerHeight - 120) * zoom / 100)}px`,
              border: 'none', borderRadius: '8px', background: 'white',
              transition: 'width 0.2s ease, height 0.2s ease'
            }}
            title="Invoice PDF"
          />
        ) : (
          <div style={{ background: 'white', borderRadius: '10px', padding: '8px', display: 'inline-block', transition: 'transform 0.2s ease', transform: `scale(${zoom / 100})`, transformOrigin: 'top center' }}>
            <img src={dataUrl} alt="Invoice"
              style={{ display: 'block', maxWidth: '900px', width: '100%', borderRadius: '6px' }} />
          </div>
        )}
      </div>
      ...
      {/* Footer info */}
      <div onClick={(e) => e.stopPropagation()}
        style={{ padding: '8px', color: 'rgba(255,255,255,0.5)', fontSize: '11px', textAlign: 'center' }}>
        {isPdf ? 'PDF Document — Use scroll to navigate pages' : 'Image'} · {zoom}% · Press Esc to close
      </div>
    </div>
  );
}
...
export default function SupplierList() {
  ...
  const downloadImage = (dataUrl, fileName) => {
    try {
      ...
      const ext = mime.includes('pdf') ? 'pdf' : mime.includes('png') ? 'png' : 'jpg';
      const a = document.createElement('a');
      a.href = url; a.download = fileName || `invoice.${ext}`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
      URL.revokeObjectURL(url);
    } catch (e) { console.error(e); }
  };
  const openViewer = (imageUrl, invNum) => { setViewingImage(imageUrl); setViewingInvNum(invNum || ''); };
  ...
  if (selectedSupplier) {
    ...
    return (<div>
                ...
                <div key={inv._id} style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                  <div style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '14px', cursor: 'pointer' }}
                    ...
                    <div style={{ width: '56px', height: '56px', borderRadius: '8px', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                      {inv.imageUrl && !isPdf ? <img src={inv.imageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        : isPdf ? <div style={{ textAlign: 'center' }}><FileText size={20} color="#dc2626" /><span style={{ fontSize: '8px', fontWeight: '700', color: '#dc2626', display: 'block' }}>PDF</span></div>
                  ...
                  {isExpanded && (
                    <div style={{ borderTop: '1px solid #e2e8f0' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', background: '#f8fafc', padding: '8px 16px', fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                      ...
                      {inv.imageUrl && (
                        <div style={{ padding: '12px 16px', borderTop: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <div style={{ width: '50px', height: '50px', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc' }}
                            onClick={() => openViewer(inv.imageUrl, inv.invoiceNumber)}>
                            {isPdf ? <div style={{ textAlign: 'center' }}><FileText size={18} color="#dc2626" /><div style={{ fontSize: '7px', fontWeight: '700', color: '#dc2626' }}>PDF</div></div>
                              : <img src={inv.imageUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                          </div>
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button onClick={() => openViewer(inv.imageUrl, inv.invoiceNumber)} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '5px 12px', background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', color: '#7c3aed', fontWeight: '500' }}><Eye size={13} /> View</button>
                            <button onClick={() => { try { const { blob, mime } = b64ToBlob(inv.imageUrl); const isPdfFile = mime.includes('pdf'); if (isPdfFile) { const url = URL.createObjectURL(blob); const w = window.open(url); if(w){w.onload=()=>{w.focus();w.print();}} } else { const w=window.open('','_blank'); if(w){w.document.write('<html><body><img src="'+inv.imageUrl+'" onload="window.print();window.close()" style="max-width:100%"/></body></html>'); w.document.close();} } } catch(e){console.error(e);} }}
                              style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '5px 12px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', color: '#2563eb', fontWeight: '500' }}><Printer size={13} /> Print</button>
                            <button onClick={() => downloadImage(inv.imageUrl, `invoice-${inv.invoiceNumber || inv._id}.${isPdf?'pdf':'jpg'}`)}
                              style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '5px 12px', background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', color: '#059669', fontWeight: '500' }}><Download size={13} /> Save</button>
                          </div>
                        </div>
                      )}
                    </div>
      ...
      {viewingImage && <DocViewer dataUrl={viewingImage} onClose={() => setViewingImage(null)} invoiceNumber={viewingInvNum} />}
  ...
  return (<div>
    ...
    : suppliers.length === 0 ? (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
        {suppliers.map((sup) => (
          <div key={sup._id} onClick={() => openSupplier(sup)}
            style={{ background: 'white', borderRadius: '12px', padding: '20px', border: '1px solid #e2e8f0', cursor: 'pointer', transition: 'all 0.15s' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <div style={{ fontSize: '15px', fontWeight: '600', color: '#0f172a' }}>{sup.name}</div>
                  <div style={{ display: 'flex', gap: '4px', marginTop: '2px' }}>{Array.from({length:5}).map((_,i)=>(<Star key={i} size={12} fill={i<(sup.rating||5)?'#f59e0b':'none'} color={i<(sup.rating||5)?'#f59e0b':'#e2e8f0'}/>))}</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '6px' }} onClick={(e) => e.stopPropagation()}>
                <button onClick={(e) => handleEdit(sup, e)} style={{ padding: '6px', background: '#eef2ff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}><Edit size={14} color="#4338ca" /></button>
                <button onClick={(e) => handleDelete(sup._id, e)} style={{ padding: '6px', background: '#fef2f2', border: 'none', borderRadius: '6px', cursor: 'pointer' }}><Trash2 size={14} color="#dc2626" /></button>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '12px', fontSize: '12px', color: '#64748b' }}>
              {sup.contact?.phone && <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Phone size={12} /> {sup.contact.phone}</div>}
              {sup.contact?.email && <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Mail size={12} /> {sup.contact.email}</div>}
              {sup.address?.city && <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><MapPin size={12} /> {sup.address.city}{sup.address.state?`, ${sup.address.state}`:''}</div>}
            </div>
            <div style={{ display: 'flex', gap: '12px', padding: '10px 0', borderTop: '1px solid #f1f5f9' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px' }}><FileText size={13} color="#6366f1" /><span style={{ fontWeight: '600', color: '#334155' }}>{sup.totalInvoices||sup.invoices?.length||0}</span><span style={{ color: '#94a3b8' }}>invoices</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px' }}><DollarSign size={13} color="#10b981" /><span style={{ fontWeight: '600', color: '#334155' }}>${(sup.totalSpent||0).toLocaleString()}</span><span style={{ color: '#94a3b8' }}>total</span></div>
            </div>
          </div>
        ))}
      </div>
...
echo "✅ Done - Zoom fixed + Print added"
