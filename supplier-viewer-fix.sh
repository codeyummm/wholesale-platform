#!/bin/bash
echo "🔧 Fixing download + image viewer..."
...
export default function SupplierList() {
  ...
  // Fixed download: convert base64 to blob and trigger proper download
  ...
  if (selectedSupplier) {
    ...
    return (<div>
      ...
      {/* FIXED Image Viewer - white background container, no dark overlay on image */}
      {viewingImage && (
        <div onClick={() => setViewingImage(null)}
          style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }}>
          <div onClick={(e) => e.stopPropagation()}
            style={{ position: 'relative', background: 'white', borderRadius: '16px', padding: '8px', maxWidth: '95vw', maxHeight: '95vh', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
            {/* Buttons */}
            <div style={{ position: 'absolute', top: '-18px', right: '-18px', display: 'flex', gap: '8px', zIndex: 10000 }}>
              <button onClick={() => downloadImage(viewingImage, 'invoice.jpg')}
                style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#10b981', border: '2px solid white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}>
                <Download size={18} color="white" />
              </button>
              <button onClick={() => setViewingImage(null)}
                style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#ef4444', border: '2px solid white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}>
                <X size={18} color="white" />
              </button>
            </div>
            <img src={viewingImage} alt="Invoice"
              style={{ display: 'block', maxWidth: '90vw', maxHeight: '88vh', borderRadius: '12px', objectFit: 'contain' }} />
          </div>
        </div>
      )}
...
echo "✅ SupplierList fixed - download + image viewer"
