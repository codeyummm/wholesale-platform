  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (priceMatch) {
      ...
      if (description && price > 0) {
        ...
        // Generate placeholder IMEIs for each device
        const imeis = [];
        for (let k = 0; k < qty; k++) {
          imeis.push('555500000000000');
        }
    ...
    if (addToInventory && products && products.length > 0) {
      ...
      for (const product of products) {
        try {
          ...
          console.log('Processing inventory item:', { modelName, brandName, qty, imeisCount: imeis.length });
          
          // Create devices array with IMEIs
          const devices = [];
          for (let i = 0; i < qty; i++) {
            devices.push({
              imei: imeis[i] || `555500000000${String(Date.now()).slice(-3)}${String(i).padStart(3, '0')}`,
              unlockStatus: product.lockStatus?.toLowerCase() === 'locked' ? 'locked' : 'unlocked',
              condition: 'used',
              grade: product.grade || 'Grade 5',
              isSold: false
            });
          }
          ...
          if (existingItem) {
            ...
            existingItem.devices.push(...devices);
