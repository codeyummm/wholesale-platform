...
import { X, Globe, AlertCircle, ExternalLink, CheckCircle2 } from 'lucide-react';
...
export default function SyncModal({ isOpen, onClose, listing, onSyncStarted }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 transition-opacity">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden transition-all transform duration-300">
        <div className="flex justify-between items-center p-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <Globe className="text-indigo-600" size={20} />
            Sync Listing
          </h2>
          <button onClick={() => { setSuccess(false); onClose(); }} className="text-gray-400 hover:text-gray-600">
            <X size={20} />
          </button>
        </div>
        ...
        {success ? (
          <div className="p-12 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <h3 className="text-xl font-bold text-gray-900">Sync job started!</h3>
            <p className="text-gray-500 text-sm">Your listing is being pushed to {platform}.</p>
          </div>
        ) : (
          <>
