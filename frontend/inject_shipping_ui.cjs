...
const defaultSettingsRegex = /ebay:    \{ categoryId: '9355', conditionId: '3000', returnProfileId: '', shippingProfileId: '', paymentProfileId: '' \},/;
const newDefaultSettings = `ebay:    { categoryId: '9355', conditionId: '3000', returnProfileId: '', shippingProfileId: '', paymentProfileId: '', shipping: { weightLbs: '', weightOz: '', length: '', width: '', height: '', irregularPackage: false, costType: 'Flat: Same cost to all buyers', primaryService: 'USPS Ground Advantage', buyerPays: '12.00', freeShipping: false } },`;
if (content.match(defaultSettingsRegex)) {
  content = content.replace(defaultSettingsRegex, newDefaultSettings);
} else {
  console.log("Could not find initial state regex.");
}

// 2. Replace Fulfillment block
const startTag = '{/* Fulfillment */}';
const endTag = '              </CardContent>';

const startIndex = content.indexOf(startTag);
const endIndex = content.indexOf(endTag, startIndex);

if (startIndex === -1 || endIndex === -1) {
  console.log("Could not find Fulfillment block boundaries.");
  process.exit(1);
}

const beforeBlock = content.substring(0, startIndex);
const afterBlock = content.substring(endIndex); // Keeps the </CardContent>
                ...
                <div className="mt-6 space-y-6">
                  ...
                  <div className="border border-gray-100 rounded-lg p-4 bg-gray-50/50">
                       ...
                       <div>
                         ...
                         <div className="flex items-center space-x-2">
                           <div className="relative w-24">
                             <Input type="number" className="h-9 pr-8" placeholder="0" value={formData.platformSettings.ebay.shipping?.weightLbs || ''} onChange={e => {
                                const newShipping = { ...(formData.platformSettings.ebay.shipping || {}), weightLbs: e.target.value };
                           ...
                           <div className="relative w-24">
                             <Input type="number" className="h-9 pr-8" placeholder="0" value={formData.platformSettings.ebay.shipping?.weightOz || ''} onChange={e => {
                                const newShipping = { ...(formData.platformSettings.ebay.shipping || {}), weightOz: e.target.value };
                                handlePlatformChange('ebay', 'shipping', newShipping);
                             }} />
                       ...
                       <div>
                         ...
                         <div className="flex items-center space-x-2">
                           <div className="relative w-20">
                             <Input type="number" className="h-9 pr-6" placeholder="L" value={formData.platformSettings.ebay.shipping?.length || ''} onChange={e => {
                                const newShipping = { ...(formData.platformSettings.ebay.shipping || {}), length: e.target.value };
                                handlePlatformChange('ebay', 'shipping', newShipping);
                             }} />
                           ...
                           <div className="relative w-20">
                             <Input type="number" className="h-9 pr-6" placeholder="W" value={formData.platformSettings.ebay.shipping?.width || ''} onChange={e => {
                                const newShipping = { ...(formData.platformSettings.ebay.shipping || {}), width: e.target.value };
                                handlePlatformChange('ebay', 'shipping', newShipping);
                             }} />
                             <span className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 text-xs">in.</span>
                           </div>
                           <span className="text-gray-400 text-sm">x</span>
                           <div className="relative w-20">
                             <Input type="number" className="h-9 pr-6" placeholder="H" value={formData.platformSettings.ebay.shipping?.height || ''} onChange={e => {
                                const newShipping = { ...(formData.platformSettings.ebay.shipping || {}), height: e.target.value };
                                handlePlatformChange('ebay', 'shipping', newShipping);
                             }} />
                             <span className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 text-xs">in.</span>
                           </div>
                     ...
                     <label className="flex items-start space-x-2 text-sm text-gray-800 cursor-pointer">
                       <input type="checkbox" className="rounded border-gray-400 text-blue-600 focus:ring-blue-500 mt-0.5" checked={formData.platformSettings.ebay.shipping?.irregularPackage || false} onChange={e => {
                          const newShipping = { ...(formData.platformSettings.ebay.shipping || {}), irregularPackage: e.target.checked };
                          handlePlatformChange('ebay', 'shipping', newShipping);
                       }} />
                       <div>
                  ...
                  <div className="border border-gray-100 rounded-lg p-4 bg-gray-50/50">
                     ...
                     <div className="mb-6 w-full md:w-1/2">
                       ...
                       <select value={formData.platformSettings.ebay.shipping?.costType || 'Flat: Same cost to all buyers'} onChange={e => {
                          const newShipping = { ...(formData.platformSettings.ebay.shipping || {}), costType: e.target.value };
                          handlePlatformChange('ebay', 'shipping', newShipping);
                         ...
                         <option value="Flat: Same cost to all buyers">Flat: Same cost to all buyers</option>
                         <option value="Calculated: Cost varies by buyer location">Calculated: Cost varies by buyer location</option>
                       </select>
                     </div>
                         ...
                         <div className="mt-4 md:mt-0 flex items-center gap-3 border-l border-gray-100 pl-4 ml-4">
                           <div>
                             ...
                             <div className="relative w-32">
                               ...
                               <Input type="number" className="h-10 pl-7 text-sm font-medium" placeholder="0.00" value={formData.platformSettings.ebay.shipping?.buyerPays || ''} onChange={e => {
                                  const newShipping = { ...(formData.platformSettings.ebay.shipping || {}), buyerPays: e.target.value };
                                  handlePlatformChange('ebay', 'shipping', newShipping);
                               }} />
                             </div>
                           </div>
                  ...
                  {/* Fulfillment (Legacy Fields) */}
                  <div className="border border-gray-100 rounded-lg p-4 bg-gray-50/50">
                    <h4 className="font-semibold text-sm text-gray-700 mb-3">Legacy Fulfillment Profiles</h4>
...
content = beforeBlock + newShippingUI + afterBlock;
fs.writeFileSync('frontend/src/components/Listings/ListingEditor.jsx', content);
console.log('Successfully injected Shipping UI!');
