...
import { Plus, Search, Trash2, Edit, Camera, ChevronDown, ChevronUp, X, Save, Smartphone, Printer, ShoppingBag } from 'lucide-react';
import QRCode from 'qrcode';
import Barcode from 'react-barcode';
import InvoiceScanner from '../InvoiceScanner';
import EbayListModal from './EbayListModal';
...
export default function InventoryList() {
  ...
  const printLabel = () => {
    ...
    printWindow.document.write('<html><head><title>Print Label</title><style>@page { size: 2in 1in; margin: 0; } body { margin: 0; padding: 0; } .label { width: 2in; height: 1in; padding: 4px; box-sizing: border-box; font-family: Arial, sans-serif; }</style></head><body>' + printContent.innerHTML + '</body></html>');
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 relative">
          ...
          <input type="text" placeholder="Search by model, brand, or IMEI..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
        </div>
                ...
                {inventory.map((item) => {
                  ...
                  return (
                    <React.Fragment key={item._id}>
                      <tr style={{ borderBottom: '1px solid #e5e7eb', background: item.ebayListingId ? '#ecfdf5' : (isExpanded ? '#f9fafb' : 'white'), transition: 'background-color 0.2s ease' }}>
                        <td style={{ padding: '1rem' }}>
                          <button onClick={() => toggleRow(item._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem' }}>
                        ...
                        <td style={{ padding: '1rem' }}>
                          <span style={{ padding: '0.25rem 0.75rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: '500', background: available < 10 ? '#fee2e2' : '#d1fae5', color: available < 10 ? '#991b1b' : '#065f46' }}>{available} / {item.quantity}</span>
                        </td>
                        <td style={{ padding: '1rem' }}>
                        ...
                        <td style={{ padding: '1rem' }}>
                          <div style={{ display: 'flex', gap: '0.5rem' }}>
                            {item.ebayListingId ? (
                              <a href={`https://www.ebay.com/itm/${item.ebayListingId}`} target="_blank" rel="noopener noreferrer" style={{ background: '#d1fae5', border: 'none', color: '#065f46', padding: '0.5rem', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title={`Listed on eBay (ID: ${item.ebayListingId}). Click to view.`}>
                                <ShoppingBag size={18} />
                              </a>
                            ) : (
                              <button onClick={() => { setSelectedEbayItem(item); setShowEbayModal(true); }} style={{ background: '#f3f4f6', border: 'none', color: '#4b5563', cursor: 'pointer', padding: '0.5rem', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="List on eBay">
                                <ShoppingBag size={18} />
                              </button>
                            )}
                            <button onClick={() => handleEdit(item)} style={{ background: '#eff6ff', border: 'none', color: '#2563eb', cursor: 'pointer', padding: '0.5rem', borderRadius: '4px' }} title="Edit Item"><Edit size={18} /></button>
                            <button onClick={() => handleDelete(item._id)} style={{ background: '#fef2f2', border: 'none', color: '#ef4444', cursor: 'pointer', padding: '0.5rem', borderRadius: '4px' }} title="Delete Item"><Trash2 size={18} /></button>
                          </div>
                        </td>
                      </tr>
                      {isExpanded && (
                        <tr>
                          <td colSpan="6" style={{ padding: '0', background: '#f9fafb' }}>
                            <div style={{ padding: '1rem 2rem', borderBottom: '1px solid #e5e7eb' }}>
                              <h4 style={{ margin: '0 0 1rem', fontSize: '0.9rem', fontWeight: '600', color: '#374151' }}>Devices ({item.devices?.length || 0})</h4>
                              {item.devices && item.devices.length > 0 ? (
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '0.75rem' }}>
                                  {item.devices.map((device, idx) => (
                                    <div key={idx} style={{ background: 'white', padding: '0.75rem', borderRadius: '8px', border: `1px solid ${device.isSold ? '#fecaca' : '#d1fae5'}`, opacity: device.isSold ? 0.7 : 1 }}>
                                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                                        <span style={{ fontSize: '0.75rem', fontWeight: '600', color: '#6b7280' }}>#{idx + 1}</span>
                                        <span style={{ fontSize: '0.7rem', padding: '2px 8px', borderRadius: '4px', background: device.isSold ? '#fee2e2' : '#d1fae5', color: device.isSold ? '#991b1b' : '#065f46' }}>{device.isSold ? 'Sold' : 'Available'}</span>
                                      </div>
                                      <div style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: '#1f2937', marginBottom: '0.5rem' }}>IMEI: {device.imei || 'N/A'}</div>
            ...
            <div style={{ background: 'white', borderRadius: '0.5rem', padding: '2rem', maxWidth: '600px', width: '100%', maxHeight: '90vh', overflow: 'auto' }}>
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                  <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Model *</label><input type="text" required value={formData.model} onChange={(e) => setFormData({ ...formData, model: e.target.value })} placeholder="iPhone 15 Pro" style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                  <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Brand *</label><input type="text" required value={formData.brand} onChange={(e) => setFormData({ ...formData, brand: e.target.value })} placeholder="Apple" style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                  <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Storage</label><input type="text" value={formData.specifications.storage} onChange={(e) => setFormData({ ...formData, specifications: { ...formData.specifications, storage: e.target.value } })} placeholder="256GB" style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                  <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Color</label><input type="text" value={formData.specifications.color} onChange={(e) => setFormData({ ...formData, specifications: { ...formData.specifications, color: e.target.value } })} placeholder="Black" style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                  <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Quantity *</label><input type="number" required min="1" value={formData.quantity} onChange={(e) => setFormData({ ...formData, quantity: parseInt(e.target.value) })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                  <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Cost Price *</label><input type="number" required min="0" step="0.01" value={formData.price.cost} onChange={(e) => setFormData({ ...formData, price: { ...formData.price, cost: parseFloat(e.target.value) } })} placeholder="999.00" style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                  <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Retail Price *</label><input type="number" required min="0" step="0.01" value={formData.price.retail} onChange={(e) => setFormData({ ...formData, price: { ...formData.price, retail: parseFloat(e.target.value) } })} placeholder="1199.00" style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                </div>
        ...
        {showEditModal && editItem && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '1rem' }}>
            <div style={{ background: 'white', borderRadius: '0.5rem', padding: '2rem', maxWidth: '800px', width: '100%', maxHeight: '90vh', overflow: 'auto' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Model</label><input type="text" value={editItem.model || ''} onChange={(e) => updateEditField('model', e.target.value)} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Brand</label><input type="text" value={editItem.brand || ''} onChange={(e) => updateEditField('brand', e.target.value)} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Storage</label><input type="text" value={editItem.specifications?.storage || ''} onChange={(e) => updateEditSpec('storage', e.target.value)} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Color</label><input type="text" value={editItem.specifications?.color || ''} onChange={(e) => updateEditSpec('color', e.target.value)} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Cost Price</label><input type="number" step="0.01" value={editItem.price?.cost || 0} onChange={(e) => updateEditPrice('cost', e.target.value)} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Retail Price</label><input type="number" step="0.01" value={editItem.price?.retail || 0} onChange={(e) => updateEditPrice('retail', e.target.value)} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
              </div>
                      ...
                      {editItem.devices.map((device, idx) => (
                        <div key={idx} style={{ background: 'white', padding: '0.75rem', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: '0.5rem' }}>
                            <div><label style={{ fontSize: '0.7rem', color: '#6b7280' }}>IMEI</label><input type="text" value={device.imei || ''} onChange={(e) => updateDeviceImei(idx, e.target.value)} style={{ width: '100%', padding: '6px', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '0.85rem', fontFamily: 'monospace' }} /></div>
                            <div><label style={{ fontSize: '0.7rem', color: '#6b7280' }}>Status</label><select value={device.unlockStatus || 'unlocked'} onChange={(e) => updateDeviceField(idx, 'unlockStatus', e.target.value)} style={{ width: '100%', padding: '6px', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '0.85rem' }}><option value="unlocked">Unlocked</option><option value="locked">Locked</option><option value="carrier_locked">Carrier</option></select></div>
                            <div><label style={{ fontSize: '0.7rem', color: '#6b7280' }}>Condition</label><select value={device.condition || 'used'} onChange={(e) => updateDeviceField(idx, 'condition', e.target.value)} style={{ width: '100%', padding: '6px', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '0.85rem' }}><option value="new">New</option><option value="refurbished">Refurbished</option><option value="used">Used</option></select></div>
                            <div><label style={{ fontSize: '0.7rem', color: '#6b7280' }}>Sold?</label><select value={device.isSold ? 'yes' : 'no'} onChange={(e) => updateDeviceField(idx, 'isSold', e.target.value === 'yes')} style={{ width: '100%', padding: '6px', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '0.85rem' }}><option value="no">No</option><option value="yes">Yes</option></select></div>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        {showDeviceEditModal && editDevice && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '1rem' }}>
            <div style={{ background: 'white', borderRadius: '0.5rem', padding: '2rem', maxWidth: '500px', width: '100%' }}>
              ...
              <div style={{ marginBottom: '1rem' }}><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>IMEI</label><input type="text" value={editDevice.imei || ''} onChange={(e) => setEditDevice({ ...editDevice, imei: e.target.value })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem', fontFamily: 'monospace' }} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Unlock Status</label><select value={editDevice.unlockStatus || 'unlocked'} onChange={(e) => setEditDevice({ ...editDevice, unlockStatus: e.target.value })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }}><option value="unlocked">Unlocked</option><option value="locked">Locked</option><option value="carrier_locked">Carrier Locked</option></select></div>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Condition</label><select value={editDevice.condition || 'used'} onChange={(e) => setEditDevice({ ...editDevice, condition: e.target.value })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }}><option value="new">New</option><option value="refurbished">Refurbished</option><option value="used">Used</option></select></div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Grade</label><input type="text" value={editDevice.grade || ''} onChange={(e) => setEditDevice({ ...editDevice, grade: e.target.value })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }} /></div>
                <div><label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', marginBottom: '0.25rem' }}>Sold Status</label><select value={editDevice.isSold ? 'yes' : 'no'} onChange={(e) => setEditDevice({ ...editDevice, isSold: e.target.value === 'yes' })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #d1d5db', borderRadius: '0.375rem' }}><option value="no">Available</option><option value="yes">Sold</option></select></div>
              </div>
              <div style={{ display: 'flex', gap: '0.75rem' }}>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '1rem' }}>
            <div style={{ background: 'white', borderRadius: '0.5rem', padding: '2rem', maxWidth: '400px', width: '100%' }}>
              ...
              <div style={{ border: '2px dashed #d1d5db', borderRadius: '8px', padding: '1rem', marginBottom: '1.5rem', background: '#f9fafb' }}>
                ...
                <div ref={labelRef} className="label" style={{ width: '192px', height: '96px', background: 'white', border: '1px solid #e5e7eb', margin: '0 auto', padding: '4px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
                    <Barcode value={labelDevice.imei || '000000000000000'} width={1} height={30} fontSize={8} margin={0} displayValue={true} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '2px' }}>
                    <div style={{ fontSize: '7px', fontWeight: 'bold', color: '#1f2937', maxWidth: '110px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{labelInventory.model}</div>
                    {qrCodeUrl && <img src={qrCodeUrl} alt="QR" style={{ width: '28px', height: '28px' }} />}
                  </div>
                </div>
              </div>
        ...
        <EbayListModal 
          isOpen={showEbayModal} 
          onClose={() => { setShowEbayModal(false); setSelectedEbayItem(null); }}
          item={selectedEbayItem}
          onSuccess={(data) => {
            setInventory(prev => prev.map(item => 
              item._id === selectedEbayItem._id 
                ? { ...item, ebayListingId: data.listingId, ebaySku: data.sku } 
                : item
            ));
          }}
        />
