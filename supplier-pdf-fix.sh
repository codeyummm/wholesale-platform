#!/bin/bash
echo "🔧 Fixing PDF viewer - native zoom/print..."

cat > frontend/src/components/Suppliers/SupplierList.jsx << 'SUPPLIEREOF'
import React, { useEffect, useState } from 'react';
import api from '../../utils/api';
import InvoiceScanner from '../InvoiceScanner';
import { Plus, Search, Edit, Trash2, Phone, Mail, MapPin, X, Save, FileText, DollarSign, Eye, Building, Star, Camera, ArrowLeft, Download, ChevronDown, ChevronUp, ZoomIn, ZoomOut, Printer } from 'lucide-react';
...
function DocViewer({ dataUrl, onClose, invoiceNumber }) {
  ...
  const downloadFile = () => {
    ...
    link.href = url; link.download = `invoice-${invoiceNumber || 'scan'}.${ext}`;
  ...
  const printFile = () => {
    if (isPdf && blobUrl) {
      ...
      if (printWindow) {
        printWindow.document.write(`<html><body style="margin:0;display:flex;justify-content:center;"><img src="${dataUrl}" style="max-width:100%;"/></body></html>`);
  ...
  const zoomIn = () => setZoom(p => Math.min(p + 25, 300));
  const zoomOut = () => setZoom(p => Math.max(p - 25, 50));
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15,23,42,0.9)', zIndex: 9999, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px 20px', background: '#1e293b', borderBottom: '1px solid #334155', flexShrink: 0 }}>
        {!isPdf && (<>
          <button onClick={zoomOut} style={tbBtn}><ZoomOut size={16} /></button>
          ...
          <button onClick={zoomIn} style={tbBtn}><ZoomIn size={16} /></button>
        ...
        {isPdf ? (
          blobUrl ? (
            <object data={blobUrl} type="application/pdf" style={{ width: '100%', height: '100%' }}>
              <iframe src={blobUrl} style={{ width: '100%', height: '100%', border: 'none' }} title="Invoice PDF" />
            </object>
      ...
      <div style={{ padding: '6px 20px', background: '#1e293b', borderTop: '1px solid #334155', textAlign: 'center', flexShrink: 0 }}>
        <span style={{ color: '#94a3b8', fontSize: '12px' }}>
          {isPdf ? 'PDF Document — Use scroll to navigate pages · Browser controls for zoom' : `Image · ${zoom}%`} · Press Esc to close
        </span>
      </div>
    </div>
  );
...
export default function SupplierList() {
  ...
  const openSupplier = async (s) => { setSelectedSupplier(s); setActiveTab('invoices'); setExpandedInvoice(null); fetchInvoicesFor(s._id); };
  ...
  const handleDelete = async (id, e) => { if (e) e.stopPropagation(); if (!window.confirm('Delete this supplier?')) return; try { await api.delete(`/suppliers/${id}`); if (selectedSupplier?._id===id) setSelectedSupplier(null); fetchSuppliers(); } catch(e){ alert('Failed to delete'); } };
  const resetForm = () => { setFormData({ name: '', contact: { email: '', phone: '', alternatePhone: '' }, address: { street: '', city: '', state: '', zipCode: '' }, notes: '', rating: 5 }); };
  const handleScanComplete = () => { setActiveTab('invoices'); fetchInvoicesFor(selectedSupplier._id); fetchSuppliers(); api.get(`/suppliers/${selectedSupplier._id}`).then(r => { if (r.data.success) setSelectedSupplier(r.data.data); }); };

  const downloadImage = (dataUrl, fileName) => {
    try { const blob = b64ToBlob(dataUrl); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = fileName; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url); } catch(e){ console.error(e); }
  };
  ...
  if (selectedSupplier) {
    ...
    return (<div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:'12px',marginBottom:'20px' }}>
        {[{label:'Invoices',value:s.totalInvoices||s.invoices?.length||0,icon:FileText,color:'#6366f1',bg:'#eef2ff'},{label:'Total Spent',value:`$${(s.totalSpent||0).toLocaleString()}`,icon:DollarSign,color:'#10b981',bg:'#ecfdf5'},{label:'Rating',value:`${s.rating||5}/5`,icon:Star,color:'#f59e0b',bg:'#fffbeb'}].map((c,i)=>(
      ...
      <div style={{ display:'flex',gap:'0',marginBottom:'16px',borderBottom:'2px solid #e2e8f0' }}>
        {[{id:'invoices',label:'Invoices',icon:FileText},{id:'scan',label:'Scan Invoice',icon:Camera}].map(tab=>(
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{display:'flex',alignItems:'center',gap:'6px',padding:'10px 20px',background:'none',border:'none',borderBottom:activeTab===tab.id?'2px solid #6366f1':'2px solid transparent',color:activeTab===tab.id?'#6366f1':'#64748b',fontWeight:activeTab===tab.id?'600':'400',cursor:'pointer',fontSize:'13px',marginBottom:'-2px'}}><tab.icon size={16}/> {tab.label}</button>
        ))}
      </div>
      ...
      {activeTab === 'invoices' && (<div>
        ...
        : supplierInvoices.length === 0 ? (
          <div style={{textAlign:'center',padding:'48px',background:'white',borderRadius:'12px',border:'1px solid #e2e8f0'}}>
            <FileText size={40} color="#cbd5e1" style={{margin:'0 auto 12px'}}/><p style={{color:'#64748b',marginBottom:'16px'}}>No invoices for this supplier yet</p>
              ...
              return (
                <div key={inv._id} style={{background:'white',borderRadius:'12px',border:'1px solid #e2e8f0',overflow:'hidden'}}>
                  <div style={{padding:'16px',display:'flex',alignItems:'center',gap:'14px',cursor:'pointer'}}
                    onClick={() => setExpandedInvoice(isExp?null:inv._id)}
                    ...
                    <div style={{display:'flex',gap:'6px',alignItems:'center'}} onClick={e=>e.stopPropagation()}>
                      {inv.imageUrl && <button onClick={() => openViewer(inv.imageUrl,inv.invoiceNumber)} style={{padding:'6px',background:'#f5f3ff',border:'none',borderRadius:'6px',cursor:'pointer'}} title="View"><Eye size={16} color="#8b5cf6"/></button>}
                      {inv.imageUrl && <button onClick={() => downloadImage(inv.imageUrl,`invoice-${inv.invoiceNumber||inv._id}.${ext}`)} style={{padding:'6px',background:'#ecfdf5',border:'none',borderRadius:'6px',cursor:'pointer'}} title="Download"><Download size={16} color="#10b981"/></button>}
                    </div>
                    {isExp?<ChevronUp size={18} color="#94a3b8"/>:<ChevronDown size={18} color="#94a3b8"/>}
                  </div>
                  {isExp && (
                    <div style={{borderTop:'1px solid #e2e8f0'}}>
                      <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',background:'#f8fafc',padding:'8px 16px',fontSize:'11px',fontWeight:'600',color:'#64748b',textTransform:'uppercase',letterSpacing:'0.05em'}}><span>Product</span><span style={{textAlign:'center'}}>Qty</span><span style={{textAlign:'right'}}>Unit Price</span><span style={{textAlign:'right'}}>Total</span></div>
                      {(inv.products||[]).map((p,idx)=>(
                        <div key={idx} style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',padding:'10px 16px',borderBottom:'1px solid #f1f5f9',fontSize:'13px'}}>
                          <div><div style={{fontWeight:'500',color:'#0f172a'}}>{p.brand} {p.model}</div><div style={{fontSize:'11px',color:'#94a3b8'}}>{[p.storage,p.color,p.grade,p.lockStatus].filter(Boolean).join(' · ')}</div></div>
                      ...
                      {inv.imageUrl && (
                        <div style={{padding:'12px 16px',borderTop:'1px solid #e2e8f0',display:'flex',alignItems:'center',gap:'12px'}}>
                          <div style={{width:'60px',height:'60px',borderRadius:'8px',border:'1px solid #e2e8f0',overflow:'hidden',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',background:'#f8fafc'}}
                            onClick={() => openViewer(inv.imageUrl,inv.invoiceNumber)}>
                            {isPdf?<div style={{textAlign:'center'}}><FileText size={20} color="#dc2626"/><div style={{fontSize:'8px',fontWeight:'700',color:'#dc2626'}}>PDF</div></div>:<img src={inv.imageUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>}
                          </div>
                          <div style={{flex:1}}>
                            <p style={{margin:'0 0 4px',fontSize:'12px',fontWeight:'500',color:'#334155'}}>Original Scanned Document</p>
                            <div style={{display:'flex',gap:'8px'}}>
                              <button onClick={() => openViewer(inv.imageUrl,inv.invoiceNumber)} style={{display:'flex',alignItems:'center',gap:'4px',padding:'4px 10px',background:'#f5f3ff',border:'1px solid #ddd6fe',borderRadius:'5px',cursor:'pointer',fontSize:'11px',color:'#7c3aed'}}><Eye size={12}/> View</button>
                              <button onClick={() => downloadImage(inv.imageUrl,`invoice-${inv.invoiceNumber||inv._id}.${ext}`)} style={{display:'flex',alignItems:'center',gap:'4px',padding:'4px 10px',background:'#ecfdf5',border:'1px solid #a7f3d0',borderRadius:'5px',cursor:'pointer',fontSize:'11px',color:'#059669'}}><Download size={12}/> Download</button>
                            </div>
                          </div>
                        </div>
                      )}
          ...
          <div key={supplier._id} onClick={() => openSupplier(supplier)} style={{background:'white',borderRadius:'12px',padding:'20px',border:'1px solid #e2e8f0',cursor:'pointer',transition:'all 0.15s ease'}}
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'12px'}}>
              <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                ...
                <div><div style={{fontSize:'15px',fontWeight:'600',color:'#0f172a'}}>{supplier.name}</div><div style={{display:'flex',gap:'4px',marginTop:'2px'}}>{Array.from({length:5}).map((_,i)=>(<Star key={i} size={12} fill={i<(supplier.rating||5)?'#f59e0b':'none'} color={i<(supplier.rating||5)?'#f59e0b':'#e2e8f0'}/>))}</div></div>
              </div>
...
echo "✅ SupplierList fixed - PDF native viewer with zoom/print/page nav"
